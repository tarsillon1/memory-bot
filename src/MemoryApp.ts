...
export class MemoryApp {
    private static processNames : string[] = process.env.PROCESS_NAMES ? JSON.parse(process.env.PROCESS_NAMES) : ["openfin"];
    ...
    private static extract(action: (...args) => any, options: { header?: string[], query?: string[]; body?: string[]; }) {
        return async (req, res, next) => {
            let params = [];
            if (options.header) options.header.forEach(param => params.push(req.header[param]));
            if (options.query) options.query.forEach(param => params.push(req.query[param]));
            if (options.body) options.body.forEach(param => params.push(req.body[param]));
    ...
    private addController(controller : Controller) {
        let addRoute = (route : Route) => {
            this.app.use(path.resolve(route.base, route.mapping),
